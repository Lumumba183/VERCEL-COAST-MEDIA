...
export default function BriefSlider() {
  ...
  return (
    <div className="w-full bg-newsbooth-red text-white shadow-[0_2px_12px_rgba(239,45,63,0.35)]">
      <div className="max-w-7xl mx-auto flex items-stretch">
        ...
        <span className="relative flex items-center gap-2 shrink-0 z-10 bg-black/25 px-4 sm:px-6 py-2.5">
          ...
          <Zap size={15} className="text-newsbooth-accent fill-newsbooth-accent" />
          ...
          <div className="animate-ticker py-2.5">
            {ticker.map((item, i) => (
              <span key={`${item.id}-${i}`} className="mx-7 text-sm sm:text-[15px] font-semibold tracking-wide">
                <span className="text-newsbooth-accent mr-3 font-extrabold">•</span>
                {item.text}
              </span>
            ))}
          </div>
