...
export default function AnalyticsPanel() {
  ...
  const cards = [
    { label: 'Visitors today', value: data.today, sub: `${data.viewsToday} page views`, icon: Users, color: 'text-newsbooth-blue' },
    { label: 'Last 7 days', value: data.week, sub: `${data.viewsWeek} page views`, icon: TrendingUp, color: 'text-emerald-600' },
    { label: 'This month', value: data.month, sub: `${data.viewsMonth} page views`, icon: Eye, color: 'text-newsbooth-accent' },
  ];
  const fmtDay = (iso: string) => new Date(iso + 'T00:00:00').toLocaleDateString('en-KE', { day: 'numeric', month: 'short' });

  return (
    <div className="mb-8">
      <h3 className="font-extrabold text-newsbooth-navy text-lg mb-4 flex items-center gap-2">
        <Activity size={18} className="text-newsbooth-blue" /> Website Traffic
      </h3>

      {/* Live now banner */}
      <div className="bg-newsbooth-navy rounded-2xl shadow-sm p-6 mb-5 flex items-center justify-between text-white">
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-5">
        {cards.map((c) => (
          <div key={c.label} className="bg-white rounded-2xl shadow-sm p-5">
            ...
            <p className="text-2xl font-extrabold text-newsbooth-navy">{c.value.toLocaleString()}</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-5">
        <p className="text-sm font-bold text-newsbooth-navy mb-4">Unique visitors — last 14 days</p>
        <div className="flex items-end gap-1.5 h-32">
          {data.daily.map((d) => (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
              <div
                className="w-full bg-newsbooth-blue/80 rounded-t hover:bg-newsbooth-blue transition min-h-[3px]"
              ...
              <div className="absolute -top-8 hidden group-hover:block bg-newsbooth-navy text-white text-[10px] px-2 py-1 rounded whitespace-nowrap z-10">
      ...
      {data.topPages.length > 0 && (
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <p className="text-sm font-bold text-newsbooth-navy mb-3">Most viewed pages (last 14 days)</p>
          <ul className="divide-y divide-gray-50">
            {data.topPages.map((p) => (
              <li key={p.path} className="py-2 flex items-center justify-between text-sm">
                ...
                <span className="text-newsbooth-navy font-extrabold shrink-0">{p.views.toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </div>
