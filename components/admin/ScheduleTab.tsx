  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h2 className="font-extrabold text-newsbooth-navy text-xl">Programme Schedule ({items.length})</h2>
        <button onClick={() => { setShowForm(true); setError(''); }} className="bg-newsbooth-red text-white font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 text-sm hover:brightness-110">
      ...
      {showForm && (
        <form onSubmit={save} className="bg-white rounded-2xl shadow-sm p-6 mb-6 space-y-4 border-2 border-newsbooth-blue/20">
          <div className="flex items-center justify-between">
            <h3 className="font-bold text-newsbooth-navy">New Programme Slot</h3>
      ...
      {DAYS_OF_WEEK.map((day) => {
        ...
        return (
          <div key={day} className="mb-6">
            <h3 className="font-bold text-newsbooth-navy mb-2">{day}</h3>
            <div className="bg-white rounded-2xl shadow-sm divide-y divide-gray-50">
              {dayItems.map((s) => (
                <div key={s.id} className="px-5 py-3.5 flex items-center gap-4">
                  <span className="text-newsbooth-red font-bold text-sm w-32 shrink-0">{s.start_time} – {s.end_time}</span>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-newsbooth-navy truncate">{s.show_name}</p>
                    {s.host && <p className="text-xs text-gray-400">{s.host}</p>}
                  </div>
                  <button onClick={() => remove(s.id)} className="p-2 text-newsbooth-red hover:bg-red-50 rounded-lg"><Trash2 size={15} /></button>
                </div>
              ))}
            </div>
          </div>
