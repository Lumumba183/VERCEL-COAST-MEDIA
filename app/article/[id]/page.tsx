...
export default async function ArticlePage({ params }: Props) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 mt-8">
      <Link href="/news" className="inline-flex items-center gap-2 text-newsbooth-blue font-semibold text-sm mb-6 hover:gap-3 transition-all">
      ...
      <article className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="p-6 md:p-10">
          <span className={`${CATEGORY_COLORS[article.category] || 'bg-newsbooth-blue'} text-white text-xs font-bold uppercase px-3 py-1 rounded`}>
            {article.category}
          </span>
          <h1 className="text-3xl md:text-4xl font-extrabold text-newsbooth-navy leading-tight mt-4 mb-4">
            {article.title}
          </h1>
      ...
      {related.length > 0 && (
        <div className="mt-10">
          <h2 className="text-xl font-extrabold text-newsbooth-navy border-l-4 border-newsbooth-red pl-3 mb-5">Related Stories</h2>
          <div className="grid sm:grid-cols-3 gap-5">
            {related.map((a) => (
              <Link key={a.id} href={`/article/${a.slug}`} className="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition group">
                <div className="h-32 bg-newsbooth-navy overflow-hidden">
                  {a.image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={a.image_url} alt={a.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-white/20 text-3xl font-extrabold">NB</div>
                  )}
                </div>
                <div className="p-4">
                  <h3 className="font-bold text-sm text-newsbooth-navy line-clamp-2 group-hover:text-newsbooth-blue transition">{a.title}</h3>
